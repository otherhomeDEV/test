		...
		<section id="bo_list" class="mb-4">
			...
			<div class="na-table d-none d-md-table w-100 mb-0">
				<div class="<?php echo $head_class ?> d-md-table-row">
					...
					<div class="d-md-table-cell nw-15 pr-md-1">
						<?php if ($is_checkbox) { ?>
					...
					<div class="d-md-table-cell nw-15 pl-2 pr-md-1">주문자</div>
					<div class="d-md-table-cell nw-4 pr-md-1"><?php echo subject_sort_link('wr_datetime', $qstr2, 1) ?>날짜</a></div>
				</div>
			</div>
				...
				<li class="d-md-table-row px-3 py-2 p-md-0 text-md-center text-muted border-bottom<?php echo $li_css;?>">
					...
					<div class="d-md-table-cell text-left py-md-2 pr-md-1">
						<div class="na-title float-md-left">
							<div class="na-item" style="margin-right : 30px;">
								<?php if ($is_checkbox) { ?>
					...
					<div style="text-align: left;">
						<?php
						$excluded_user_id = 'lucy_gongu'; // 제외할 아이디를 설정
						...
						$sql = "
							select wr_content
							 ...
							 where wr_parent = '{$list[$i]['wr_id']}'
							   and wr_is_comment = '1'
							   AND mb_id != '{$excluded_user_id}'
							 order by wr_num
							 limit {$list[$i]['wr_comment']}
						";
						$result = sql_query($sql);
            			while ($row = sql_fetch_array($result)) {
                			echo '<p>' . cut_str($row['wr_content'], 30) . '</p>';
           				}
						?>
					</div>
